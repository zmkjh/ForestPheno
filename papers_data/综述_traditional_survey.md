...
**Label efficiency.** With only 20% of fine-tuning labels, DUNIA's tree height RMSE degrades minimally from 2.0 m to 2.1 m (r=0.92), and species classification wF1 actually improves from 76.0% to 80.1%—a phenomenon the authors attribute to the KNN retrieval mechanism benefiting from reduced noise in small balanced subsets.
...
DUNIA's label efficiency is also distinctive. At 20% of fine-tuning labels, DUNIA's tree height RMSE of 2.1 m (r=0.92) remains lower than the full-label performance of all other methods. This property is particularly valuable for forest monitoring, where ground-truth measurements (e.g., field-measured tree heights) are expensive and sparse.

The primary limitation of DUNIA is its use of single-date median composite imagery as input, which prevents it from capturing phenological dynamics. This is reflected in the PASTIS crop classification result, where the zero-shot OA of 56.2% is 28 percentage points below AnySat's fine-tuned result and 28 points below the supervised SOTA of 84.2%. AnySat's native temporal support provides a decisive advantage for tasks that require distinguishing classes based on temporal trajectories.
...
**Evaluation results.** Incorporating the symptom KB improved diagnostic accuracy by an average of 16.2 percentage points at a reference budget of k=8. Improvements were consistent across crops of varying difficulty: soybean (25 classes) improved from 31.1% to 62.2%; maize (30 classes) from 42.0% to 61.4%; tomato (20 classes) from 57.5% to 72.6%; mango (4 classes) from 82.5% to 97.5%. Accuracy increased monotonically with the reference budget from k=0 to k=8. Compared to single-pass few-shot classification at the same budget, the agentic pipeline averaged 8.1 percentage points higher. Failure modes were concentrated in cases where visual ambiguity overrode KB evidence (e.g., Anthracnose vs. Charcoal rot in stem diseases, which are visually nearly identical).
