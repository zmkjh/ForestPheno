...
一个关键进展出现在 2026 年 4 月，Chen 等人在《Nature Communications》上发表了 PhenoAssistant，展示了一个基于大语言模型的多智能体系统能够以 100% 的工具选择准确率编排计算机视觉工具、统计分析和自然语言解释，用于植物表型任务 [4]。PhenoAssistant 标志着基于智能体的编排进入了植物科学领域；然而，其评估——与本综述所评述的几乎所有方法一样——是在精心构建的农业和城市数据集上进行的。这些方法在基准测试上表现优异，但几乎从未在自然森林环境中被测试过。农业表型性能与森林迁移能力之间的差距，构成了本综述的问题空间。
...
然而，所有这些结果均来自农业和城市基准。第 4.2 节记录的六个物理障碍——多层冠层遮挡、混交林退化、LiDAR 生物量饱和、地形效应、跨站点泛化崩塌和长尾物种分布——未对任何评述方法进行过实验刻画。这些方法能证明做什么（在平坦田地中分类作物）与森林需求什么（在 30° 坡度的混交林分中，于重叠冠层下检测被压制的树木，稀有类别的训练样本不足 10 个）之间的差距，在林业文献中被公认（第 4.2 节），但在 AI 表型分析文献中缺席（第 3.1–3.5 节）。
