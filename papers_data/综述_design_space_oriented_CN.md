...
```
  ...
  └──────┬───────┘         │       │        └──────────────┘
         ...
         └─────────────────┼─► ┌──────────────┐   │
                            ...
                            │
   依赖关系 (──► 单向, ◄──► 双向):
   D1: 编码器 ↔ 时间物候 (双向) — 时间信号 vs. 权重共享
   D2: 编码器 → 融合 (单向) — 输出粒度约束融合
   D3: 质量感知 → 融合 (单向) — 质量信号 → 融合控制
   D4: 智能体 ↔ 质量感知 + 融合 (双向) — 策略选择 + 反馈
   D5: 时间物候 ↔ 质量感知 (双向) — 物候影响质量预期
   D6: 智能体 → 编码器 + 时间物候 (单向) — 任务路由到解码器分支
```
