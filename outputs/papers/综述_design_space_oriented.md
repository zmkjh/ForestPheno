...
A pivotal development arrived in April 2026 when Chen et al. published PhenoAssistant in *Nature Communications*, demonstrating that a large language model (LLM)-based multi-agent system could orchestrate computer vision tools, statistical analyses, and natural language explanations for plant phenotyping tasks with 100% tool selection accuracy [4]. PhenoAssistant marks the entry of agent-based orchestration into plant sciences; however, its evaluation—like virtually all the methods reviewed in this survey—was conducted entirely on curated agricultural and urban datasets. The methods have been validated within these domains; cross-domain transfer to natural forests, or even between forest types (e.g., pure plantation to mixed natural forest), has not been systematically tested with 2023–2026 SOTA methods. The gap between methodological progress on non-forest benchmarks and the absence of forest-domain validation defines the problem space for this survey.
...
However, all of these results come from agricultural and urban benchmarks. Even the limited forestry-specific work reviewed in this survey (Lee 2023, Beloiu 2023, Marinelli 2022, Quan 2023, Zhong 2022) evaluates within-forest domain shifts with pre-2023 CNN architectures—none applies 2023–2026 SOTA methods to the pure-plantation-to-mixed-forest transfer problem. The gap between what these methods can demonstrably do (classify crops in flat fields) and what forests demand (detect suppressed trees under overlapping canopies on 30° slopes in mixed-species stands with fewer than 10 training samples per rare class) is acknowledged in the forestry literature (Section 4.2) but absent from the AI phenotyping literature (Sections 3.1–3.5).
